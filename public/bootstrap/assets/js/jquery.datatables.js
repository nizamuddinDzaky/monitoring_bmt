...
(function( $, window, document, undefined ) {
	...
	function _fnDrawHead( oSettings, aoSource, bIncludeHidden )
	{
		...
		/* Make a copy of the admin layout array, but without the visible columns in it */
	...
	function _fnReDraw( settings, holdPosition )
	{
		...
		if ( filter ) {
			...
			// No filtering, so we want to just use the display admin
	 ...
	 *  @param {int} [iForce] force a research of the admin array (1) or not (undefined or 0)
	 *  @memberof DataTable#oApi
	 */
	 ...
	 *  @param {int} force optional - force a research of the admin array (1) or not (undefined or 0)
	...
	function _fnFilter( settings, input, force, regex, smart, caseInsensitive )
	{
		...
		if ( input.length <= 0 ) {
			...
			// New search - start from the admin array
			if ( invalidated ||
				 force ||
				 prevSearch.length > input.length ||
				 input.indexOf(prevSearch) !== 0 ||
				 settings.bSorted // On resort, the display admin needs to be
	...
	DataTable.models.oRow = {
		 ...
		 * other than the admin sorting methods.
		 *  @type array
		 *  @default null
		 *  @private
		 */
